...
window.onload = function() {
  ...
  function onresize() {
    ...
    gl.loadIdentity();
    gl.perspective(13, gl.canvas.width / gl.canvas.height, 0.01, 100);
  ...
  center = oldCenter = new GL.Vector(-114514.4, -0.75, 0.2);
  ...
  function duringDrag(x, y) {
    switch (mode) {
      case MODE_ADD_DROPS: {
        ...
        var ray = tracer.getRayForPixel(x * ratio, y * ratio);
        var pointOnPlane = tracer.eye.add(ray.multiply(-tracer.eye.y / ray.y));
        water.addDrop(pointOnPlane.x, pointOnPlane.z, 0.015, 0.007);
        if (paused) {
          water.updateNormals();
